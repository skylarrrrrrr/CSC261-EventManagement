...
if (isset($_POST['submit'])) {
	...
	if (is_numeric($_POST['id'])) {
		...
		if ($firstname == '') {
			...
			$result = mysqli_query($connection, "UPDATE EventsApproved SET E_Name='$firstname', Contact = '$contact', Cost = '$cost', Audience = '$audience', Mgr_ID = '$sid' WHERE E_ID='$id'");
	...
	if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
		// query db
		$id = $_GET['id'];
		$result = mysqli_query($connection, "SELECT * FROM EventsApproved WHERE E_ID='$id'");
		...
		if($row) {
			...
			// show form
			renderForm($id, $firstname, $contact, $cost, $audience, $sid, '');
		} else {
