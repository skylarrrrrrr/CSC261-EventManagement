...
if (isset($_POST['submit'])) {
	...
	if (is_numeric($_POST['id'])) {
		...
		if ($firstname == '') {
			...
			$result = mysqli_query($connection, "UPDATE Avaliable_Space SET Spot_ID='$firstname' WHERE Event_ID='$id'");
	...
	if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
		// query db
		$id = $_GET['id'];
		$result = mysqli_query($connection, "SELECT * FROM Avaliable_Space WHERE Event_ID='$id'");
		$row = mysqli_fetch_array( $result );

		// check that the 'id' matches up with a row in the databse
		if($row) {
			// get data from db
			$firstname = $row['Spot_ID'];
