...
if (isset($_POST['submit'])) {
	...
	if (is_numeric($_POST['id'])) {
		...
		if ($firstname == '' || $leader == '') {
			...
			$result = mysqli_query($connection, "UPDATE Organization SET O_Name ='$firstname', LeaderName = '$leader' WHERE O_ID='$id'");
	...
	if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
		// query db
		$id = $_GET['id'];
		$result = mysqli_query($connection, "SELECT * FROM Organization WHERE O_ID='$id'");
		$row = mysqli_fetch_array( $result );

		// check that the 'id' matches up with a row in the databse
		if($row) {
			// get data from db
			$firstname = $row['O_Name'];
			$leader = $row['LeaderName'];

			// show form
			renderForm($id, $firstname, $leader, '');
		} else {
