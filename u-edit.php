...
if (isset($_POST['submit'])) {
	...
	if (is_numeric($_POST['id'])) {
		...
		if ($firstname == '') {
			...
			$result = mysqli_query($connection, "UPDATE Users SET Name='$firstname', U_Department = '$mname', Email = '$lastname' WHERE U_ID='$id'");
	...
	if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
		// query db
		$id = $_GET['id'];
		$result = mysqli_query($connection, "SELECT * FROM Users WHERE U_ID='$id'");
		...
		if($row) {
			// get data from db
			$firstname = $row['Name'];
			$mname =  $row['U_Department'];
			$lastname =  $row['Email'];

			// show form
			renderForm($id, $firstname, $mname, $lastname, '');
		} else {
